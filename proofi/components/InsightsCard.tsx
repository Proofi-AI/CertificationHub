...
function InsightIcon({ type }: { type: Insight["type"] }) {
  ...
  return (
    <div className="w-6 h-6 rounded-lg flex items-center justify-center shrink-0" style={{ background: `${color}18`, border: `1px solid ${color}28` }}>
      <svg className="w-3 h-3" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.8} style={{ color }}>
    ...
    <div style={{ borderTop: "1px solid var(--border)" }}>
      ...
      >
        <div className="flex items-center gap-2">
          <svg className="w-3 h-3 text-slate-400 dark:text-white/40 shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
            <path strokeLinecap="round" strokeLinejoin="round" d="M9.879 7.519c1.171-1.025 3.071-1.025 4.242 0 1.172 1.025 1.172 2.687 0 3.712-.203.179-.43.326-.67.442-.745.361-1.45.999-1.45 1.827v.75M21 12a9 9 0 11-18 0 9 9 0 0118 0zm-9 5.25h.008v.008H12v-.008z" />
          </svg>
          <span className="text-[11px] font-bold text-slate-600 dark:text-white/60">Domain gaps</span>
          {gaps.length > 0 && (
        ...
        <svg
          className={`w-3 h-3 text-slate-400 dark:text-white/30 transition-transform duration-200 ${expanded ? "rotate-180" : ""}`}
      ...
      {expanded && (
        <div className="px-4 pb-3 space-y-1.5">
          {gaps.length === 0 ? (
            <p className="text-[11px] text-slate-500 dark:text-white/50 py-1">
              Great range of domains — no obvious gaps detected.
            </p>
          ) : (
            gaps.map((gap, i) => (
              <div
              ...
              >
                <span
                  className="inline-flex items-center px-1.5 py-0.5 rounded-full text-[10px] font-bold shrink-0 mt-0.5"
                  style={{
  ...
  return (
    ...
    >
      ...
      >
        ...
        {insights.length > 0 && (
          <span className="text-[11px] font-bold px-2 py-0.5 rounded-full bg-violet-500/10 text-violet-600 dark:text-violet-400 border border-violet-500/20 mr-1">
            {insights.length}
          </span>
        )}
        <svg
              ...
              <div className="divide-y" style={{ borderColor: "var(--border)" }}>
                {insights.map((insight, i) => (
                  <div
                    ...
                    style={{ animationDelay: `${i * 50}ms`, animation: "fadeSlideIn 0.3s ease both" }}
                  >
                    <div
                      className="w-[3px] self-stretch rounded-full shrink-0 mt-0.5"
                      style={{ background: accentColor[insight.type], minHeight: 16 }}
                    />
                    <InsightIcon type={insight.type} />
                    <p className="text-[11px] text-slate-700 dark:text-white/70 leading-relaxed pt-0.5 flex-1">
                      {insight.message}
                    </p>
                  </div>
                ))}

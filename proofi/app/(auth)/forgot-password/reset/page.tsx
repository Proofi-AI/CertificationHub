...
function ResetForm() {
  ...
  useEffect(() => {
    ...
    const storedUserId = sessionStorage.getItem("fp_verified_userId");
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const res = await fetch("/api/forgot-password/reset", {
        ...
        body: JSON.stringify({ userId, email, newPassword: password }),
      });
      ...
      sessionStorage.removeItem("fp_questions");
      sessionStorage.removeItem("fp_verified_userId");
    ...
    return (
      <div className="w-full max-w-md">
        <div className="glass rounded-2xl p-8 border border-white/10 shadow-2xl text-center">
          ...
          <Link
            href="/login"
            className="inline-flex items-center justify-center gap-2 w-full bg-gradient-to-r from-violet-600 to-blue-600 hover:from-violet-500 hover:to-blue-500 text-white font-semibold py-3 px-4 rounded-xl text-sm transition-all shadow-lg shadow-violet-500/20 hover:-translate-y-0.5"
          >
            Go to sign in
          </Link>
        </div>
